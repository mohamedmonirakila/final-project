          ...
          const fetchAmountsDue = async () => {
              try {
                  ...
                  for (const patient of patients) {
                      const response = await axios.get(`http://localhost:5000/api/amountdue/${patient.id}`);
                  ...
                  <table className="table table-striped">
                    ...
                    <tbody>
                      {patients.map((patient) => (
                        <tr key={patient.id} onClick={() => window.location.href = `/visit/${visit.id}`} style={{ cursor: 'pointer' }}>
                          <td>{patient.title}</td>
                          <td>{patient.name}</td>
                          <td>{patient.age}</td>
                          <td>{patient.history}</td>
                          <td>{amountsDue[patient.id] !== undefined ? `$${amountsDue[patient.id].toFixed(2)}` : 'Loading...'}</td>
                          <td><Link to={`/file/${patient.id}`} className="btn btn-link">View Details</Link></td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
